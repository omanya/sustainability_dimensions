...
# for( i in 1:nrow(U)){
#   print(paste("Topic ",i))
#   tnames_long[i]<-paste(rownames(what[ind,])[order(U[i,],decreasing=T)][1:20], collapse=" ")
...
```{r}
 topwords<-NULL
    for(i in 1:nrow(out$U)){
      topwords<-cbind(topwords,rownames(what)[order(out$U[i,],decreasing=T)][1:20])
    }
...
for( i in 1:nrow(out$U)){
  print(paste("Topic ",i))
  print(rownames(what[ind,])[order(out$U[i,],decreasing=T)][1:20])
}
...
## Wassestein weights and distances

```{r}
# aggregate dtm
firm_names<-sapply(rownames(dtm),function(x){return(strsplit(x,"/")[[1]][3])})
firm_names<-sapply(firm_names,function(x){substr(x,1,nchar(x)-4)})
firm_names[grepl("_s",firm_names)]<-substr(firm_names[grepl("_s",firm_names)],1,nchar(firm_names[grepl("_s",firm_names)])-2)
# save(firm_names,file="firm_names.RData")

dtm_agg<-aggregate(dtm,by=list(firm_names),sum)
rownames(dtm_agg)<-dtm_agg$Group.1
dtm_agg<-dtm_agg[,-1]

#word dist for the reports
dtm_agg<-dtm_agg/matrix(apply(dtm_agg,1,function(x){sum(as.numeric(x))}),nrow(dtm_agg), ncol(dtm_agg))

#word dist for the goals
dtm2_agg<-dtm2
dtm2_agg<-dtm2_agg/matrix(apply(dtm2_agg,1,function(x){sum(as.numeric(x))}),nrow(dtm2_agg), ncol(dtm2_agg))
goals<-sapply(rownames(dtm2),function(x){return(strsplit(x,"/")[[1]][4])})
goals<-sapply(goals,function(x){substr(x,1,nchar(x)-4)})
rownames(dtm2_agg)<-goals
dtm2_agg<-dtm2_agg[match(goals,paste0("Goal_",1:17)),]
```

compute the cost matrix based on the word-topic embeddings and the wasserstein distance

```{r}
Cmat<-Csim<-matrix(0,ncol(dtm_agg),ncol(dtm2_agg))
#cost mat
for(w in 1:ncol(dtm_agg)){
  for(ww in 1:ncol(dtm_agg)){
       if(w!=ww){
          Cmat[w,ww]<-sum((out$U[,w] - out$U[,ww])^2)
          Csim[w,ww]<-1-sum(out$U[,w]*out$U[,ww])/sqrt(sum(out$U[,w]^2)*sum(out$U[,ww]^2))
      }

  }
}
save(Cmat, file="Cmat.RData")
save(Csim, file="Csim.RData")
```

```{r}
#wasserstein dist
Wdist<-Wdist2<-matrix(NA,nrow(dtm2_agg),nrow(dtm_agg))
for(i in 1:nrow(dtm2_agg)){
  print(i)
  for(j in 1:nrow(dtm_agg)){
    #opt transport map
    ot<-transport::transport(as.numeric(dtm2_agg[i,]),as.numeric(dtm_agg[j,]),Csim,fullreturn=T)
    Wdist[i,j]<-ot$cost
    ot<-transport::transport(as.numeric(dtm2_agg[i,]),as.numeric(dtm_agg[j,]),Cmat,fullreturn=T)
    Wdist2[i,j]<-sqrt(ot$cost)
  }
}
save(Wdist, file="Wdist.RData")
save(Wdist2, file="Wdist2.RData")
```

@lee-etal-2022 use cosine dissimilarity as cost for OT distance calculation (contextualized token mover’s distance,CMD):

\[C_{i,j}^{CMD}=1-\cos(x_i, x_j)\]

with $x_\cdot$ denote the topic-term embeddings.

Find an optimal transport plan $T^*$, such that:
\[\begin{align*}\min_{T} &\sum_i\sum_j T_{ij}C_{ij},\\
s.t. &T\mathbf 1_n = \mathbf p, T^\top \mathbf 1_n = \mathbf q\end{align*}\]

where $T\in\mathbb R^{n\times n}$ is the transport plan matrix and $\mathbf p,\mathbf q$ are (term) probability vectors.
...
b1<-matrix(weights,,1)
#M2
weights<-c(0,0,0,20,0,0,0,0,0,0,0,10,0,0,0,10,0)
weights<-weights/sum(weights)
b2<-matrix(weights,,1)
#M3
weights<-c(0,0,0,40,0,0,0,0,0,0,0,10,0,0,0,10,0)
weights<-weights/sum(weights)
b3<-matrix(weights,,1)
#M4
weights<-c(0,0,0,60,0,0,0,0,0,0,0,10,0,0,0,10,0)
weights<-weights/sum(weights)
b4<-matrix(weights,,1)
#M5
weights<-c(0,0,0,80,0,0,0,0,0,0,0,10,0,0,0,10,0)
weights<-weights/sum(weights)
b5<-matrix(weights,,1)
mygoals<-t(cbind(t(dtm2_agg)%*%b1,t(dtm2_agg)%*%b2,t(dtm2_agg)%*%b3,t(dtm2_agg)%*%b4,t(dtm2_agg)%*%b5))
```

```{r}
myWdist<-myWdist2<-matrix(0,nrow(mygoals),nrow(dtm_agg))
for(i in 1:nrow(mygoals)){
  print(i)
  for(j in 1:nrow(dtm_agg)){
    #opt transport map
    ot<-transport::transport(as.numeric(mygoals[i,]),as.numeric(dtm_agg[j,]),Csim,fullreturn=T)
    myWdist[i,j]<-ot$cost
    ot<-transport::transport(as.numeric(mygoals[i,]),as.numeric(dtm_agg[j,]),Cmat,fullreturn=T)
    myWdist2[i,j]<-sqrt(ot$cost)
  }
}
#save(myWdist, file="myWdist.RData")
```
...
coords<-seq(0,ncol(Wdist_sc),5)/(ncol(Wdist_sc))
coords2<-c(0,seq(1,nrow(mygoals),1)/nrow(mygoals))
names0<-sapply(firm_names,function(x){strsplit(x,"_")[[1]][1]})
names1<-names0[!duplicated(names0)]
names2<-paste0("M",1:nrow(mygoals))
