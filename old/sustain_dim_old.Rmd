...
compute the marginal probs $p(w)$.
...
ind<-TRUE
lam=100#225
out<-compute_proj(M=what, C=what2,K=10,lam=lam) # K=9, lam=200
...
# }
plot(lams,cohs$coh[which(Ks==6),], type="l",col=1, ylim=c(-5,-2))
...
cols_dist<-sapply(colnames(dist),function(x,split="/"){return(strsplit(x,split=split)[[1]][4])})
cols_dist<-substr(cols_dist,1,nchar(cols_dist)-4)

colnames(dist)<-cols_dist
...
plot(1:ncol(min_dist_norm),min_dist_norm[1,],col=NA,ylim=c(0,1),axes=F,ylab="",xlab="")
for(i in 1:nrow(min_dist_norm)){
  lines(1:ncol(min_dist_norm),min_dist_norm[i,],col=alpha(cols[i],satur[i]))
}
axis(2)
axis(1,at=1:17,labels=colnames(min_dist_norm),cex.axis=0.5)
legend(1,0.3,col=1:3,lty=1,legend=names[1:3],cex=0.7)
legend(10,0.3,col=4:6,lty=1,legend=names[4:6],cex=0.7)
...
```{r}
b<-matrix(c(0,0,0,0.2,0,0,0.2,0,0,0.2,0,0,0.2,0,0,0.2,0),,1)
mygoal<-t(U)%*%Q%*%b
...
set.seed(1)
#folds for reports
indgr0<-which(what>0, arr.ind=T); indgr0<-indgr0[sample.int(nrow(indgr0),replace=F),]
nfolds<-10
folds<-c(seq(1,nrow(indgr0),floor(nrow(indgr0)/nfolds)+1),nrow(indgr0))
#folds for goals
indgr02<-which(what2[indgr0[,1],]>0, arr.ind=T)[,2]; #indgr02<-indgr02[sample.int(nrow(indgr02),replace=F)]
nfolds<-10
folds2<-c(seq(1,length(indgr02),floor(length(indgr02)/nfolds)+1),length(indgr02))
...
for(i in 1:length(Ks)){
  for(j in 1:nfolds){
    ...
    M_fold<-what[-indgr0[folds[j]:(folds[j+1]-1),][,1],]
    C_fold<-what2[-indgr0[folds[j]:(folds[j+1]-1),][,1],]
    out<-compute_proj(M=M_fold, C=C_fold, K=Ks[i],Kt=0,lam=lams[l])
    V=out$V
    Q=out$Q
    M_fold<-what[,-indgr0[folds[j]:(folds[j+1]-1),][,2]]
    C_fold<-what2[,-indgr02[folds2[j]:(folds2[j+1]-1)]]
    out<-compute_proj(M=M_fold, C=C_fold, K=Ks[i],Kt=0,lam=lams[l])
    U=out$U
    Mfold<-what[indgr0[folds[j]:(folds[j+1]-1),][,1],indgr0[folds[j]:(folds[j+1]-1),][,2]]
    Cfold<-what2[indgr0[folds[j]:(folds[j+1]-1),][,1],indgr02[folds2[j]:(folds2[j+1]-1)]]
    err<-c(err,c((what-t(U)%*%V)[indgr0[folds[j]:(folds[j+1]-1),][,1],indgr0[folds[j]:(folds[j+1]-1),][,2]]))
    err2<-c(err2,c((what2-t(U)%*%Q)[indgr02[folds2[j]:(folds2[j+1]-1),][,1],indgr02[folds2[j]:(folds2[j+1]-1),][,2]]))
  }
  errs[[i]]<-err
  errs2[[i]]<-err2
}
