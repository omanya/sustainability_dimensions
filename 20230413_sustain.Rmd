...
ind<-TRUE
lam=300#225
out<-compute_proj(M=what, C=what2,K=8,Kt=0,lam=lam) # K=9, lam=200
...
```{r}
coh<-mse<-prop<-prop2<-matrix(NA,length(Ks), length(lams))
for (k in 1:length(Ks)){
  for(l in 1:length(lams)){
    ...
    coh[k,l]<-mean(cohi)
    mse[k,l]<-(outt$losses[1]/prod(dim(what)))+(outt$losses[2]/prod(dim(what2)))
...
cohs<-list(coh=coh,mse=mse,prop=prop,prop2=prop2)
save(coh, file="cohs.RData")
...
#folds for goals
indgr02<-which(what>0, arr.ind=T); indgr02<-indgr02[sample.int(nrow(indgr02),replace=F),]
nfolds<-10
folds2<-c(seq(1,nrow(indgr02),floor(nrow(indgr02)/nfolds)+1),nrow(indgr02))
